...
// Otherwise, proceed as normal.
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (strlen($_POST["username"]) < 1) {
            $content .= "Your username is too short. Make sure your username is at least 1 character in length.";
        }
        // Make sure their username isn't too long.
        elseif (strlen($_POST["username"]) > 32) {
            $content .= "Your username is too long. Make sure your username is no more than 32 characters in length.";
        }
        // Make sure their username isn't taken.
        elseif ($usernameCheck->num_rows > 0) {
            $content .= "Your username is already taken. Try entering another one.";
        }
        // Make sure their email isn't too short.
        elseif (strlen($_POST["email"]) < 1) {
            $content .= "Your email address is too short. Make sure your email address is at least 1 character in length.";
        }
        // Make sure their email isn't too long.
        elseif (strlen($_POST["email"]) > 64) {
            $content .= "Your email address is too long. Make sure your email address is no more than 64 characters in length.";
        }
        // Make sure their email is valid.
        elseif (!filter_var($_POST["email"], FILTER_VALIDATE_EMAIL)) {
            $content .= "Your email address is invalid. Please try entering a valid email address.";
        }
        // Make sure their email isn't taken.
        elseif ($emailCheck->num_rows > 0) {
            $content .= "Your email address is already taken. Try entering another one.";
        }
        // Make sure their password is long enough.
        elseif (strlen($_POST["password"]) < 8) {
            $content .= "Your password isn't long enough. Make sure your password is at least 8 characters in length.";
        }
